...
class MainScreenFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val sessionManager = GazpromSharedPreferencesSessionManager(activity?.getSharedPreferences("gazprom_session.xml", Context.MODE_PRIVATE)!!)
        gazpromClient = GazpromClient(sessionManager.session)
    ...
    private fun getTransactions() {
        if (gazpromClient == null) {
            return
        }
        GlobalScope.launch (Dispatchers.Main) {
            try {
                allTransactions = gazpromClient!!.getTransactionsByDates(dateFrom!!, dateTo!!)
                updateTransactions()
                updateSelectDateToText()
            } catch (err: Exception) {
                Toast.makeText(context, "Произошла ошибка при получении транзакций: ${err}", Toast.LENGTH_SHORT).show()
            }
        }
    }
